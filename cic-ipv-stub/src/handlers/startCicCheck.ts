...
async function encrypt(
  ...
  return (
    `${protectedHeader}.` +
    `${base64url.encode(new Uint8Array(Buffer.from(new Uint8Array(encryptedKey))))}.` +
    `${base64url.encode(
      new Uint8Array(Buffer.from(new Uint8Array(initialisationVector)))
    )}.` +
    `${base64url.encode(new Uint8Array(Buffer.from(ciphertext)))}.` +
    `${base64url.encode(new Uint8Array(Buffer.from(tag)))}`
  );
}
