...
export const handler = async (
  ...
  if (event.queryStringParameters?.error != null) {
    ...
    const token = await tokenPost(
      ...
      constants.DEV_IPV_CIC_STUB_URL! + "/credential-issuer/callback?id=claimedIdentity",
      assertationJwt.data,
    ...
    const rawBody =
      vc.data["https://vocab.account.gov.uk/v1/credentialJWT"][0].split(".")[1];
    return {
      statusCode: 200,
      body: decodeRawBody(rawBody),
    };
  }
  ...
  async function generateTokenAssertation(
    ...
    const path = constants.DEV_IPV_CIC_STUB_URL! + "/generate-token-request";
  ...
  async function tokenPost(
    ...
    try {
      const postRequest = await axios.post(
        constants.DEV_CRI_CIC_API_URL + path,
  ...
  function decodeRawBody(rawBody: any): string {
    return jwtUtils.base64DecodeToString(rawBody.replace(/\W/g, ""));
  }
};
