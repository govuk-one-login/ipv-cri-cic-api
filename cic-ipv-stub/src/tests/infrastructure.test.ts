...
describe('IPV Stub Infrastructure', () => {
  ...
  beforeAll(() => {
    // @ts-expect-error
    const templateYaml: any = load(readFileSync(
      path.resolve('./template.yaml'), 'utf-8'), { schema })
    // @ts-expect-error
  ...
  it('All Lambdas must have an associated LogGroup named after their FunctionName.', () => {
    ...
    const lambdaList = Object.keys(lambdas)
    lambdaList.forEach(lambda => {
      const functionName = lambdas[lambda].Properties.FunctionName['Fn::Sub'] as string
