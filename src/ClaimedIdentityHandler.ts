		...
		try {
			...
			if (event.headers) {
				...
				if (sessionId) {
					...
					return Response(HttpCodesEnum.BAD_REQUEST, "Missing header: x-govuk-signin-session-id is required");
				}
			} else {
			...
			if (event.body) {
				return await ClaimedIdRequestProcessor.getInstance().processRequest(event, sessionId);
			} else {
