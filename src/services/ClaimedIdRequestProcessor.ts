...
export class ClaimedIdRequestProcessor {
	...
	async processRequest(event: APIGatewayProxyEvent, sessionId: string): Promise<APIGatewayProxyResult> {
		...
		try {
			...
			await this.validationHelper.validateModel(cicSession);
			logger.debug({ message: "CIC Session is", cicSession });
		} catch (error) {
			logger.error("Missing mandatory fields in the request payload", {
				error,
		...
		if (session != null) {
			...
			switch (session.authSessionState) {
			  ...
			  case AuthSessionState.CIC_ACCESS_TOKEN_ISSUED:
					logger.info(`Duplicate request for session in state: ${session.authSessionState}`, sessionId);
					return Response(HttpCodesEnum.OK, "Request already processed");
				default:
					logger.error(`Session is in an unexpected state: ${session.authSessionState}, expected state should be ${AuthSessionState.CIC_SESSION_CREATED}, ${AuthSessionState.CIC_DATA_RECEIVED}, ${AuthSessionState.CIC_AUTH_CODE_ISSUED} or ${AuthSessionState.CIC_ACCESS_TOKEN_ISSUED}`, { 
