...
import AWS from "@aws-sdk/client-kms";
...
class JwksHandler implements LambdaInterface {
    ...
    async handler(): Promise<string> {
        ...
        const jwks = await Promise.all(
            kmsKeyIds.map(async id => await getAsJwk(id, logger))
        )
        jwks.forEach(jwk => {
            if (jwk != null) {
                body.keys.push(jwk)
            } else logger.warn('Environment contains missing keys')
        })
...
const getAsJwk = async (keyId: string, logger: Logger): Promise<Jwk | null> => {
    let kmsKey
    try {
