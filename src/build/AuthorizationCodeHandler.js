...
const POWERTOOLS_SERVICE_NAME = process.env.POWERTOOLS_SERVICE_NAME ? process.env.POWERTOOLS_SERVICE_NAME : Constants_1.Constants.AUTHORIZATION_LOGGER_SVC_NAME;
...
class AuthorizationCodeHandler {
    async handler(event, context) {
        switch (event.resource) {
            case ResourcesEnum_1.ResourcesEnum.AUTHORIZATION:
                if (event.httpMethod === HttpVerbsEnum_1.HttpVerbsEnum.GET) {
                    let sessionId;
                    try {
                        logger.info("Event received", { event });
                        if (event.headers) {
                            sessionId = event.headers[Constants_1.Constants.SESSION_ID];
                            if (sessionId) {
                                ...
                                return new Response_1.Response(HttpCodesEnum_1.HttpCodesEnum.BAD_REQUEST, "Missing header: session-id is required");
                            }
                        }
                        else {
                            return new Response_1.Response(HttpCodesEnum_1.HttpCodesEnum.BAD_REQUEST, "Empty headers");
                        }
                        return await AuthorizationRequestProcessor_1.AuthorizationRequestProcessor.getInstance(logger, metrics).processRequest(event, sessionId);
                    }
                    catch (err) {
...
], AuthorizationCodeHandler.prototype, "handler", null);
